...
export function getRecipeByName(name) {
    return async function(dispatch) {
        try {
            const response = await axios.get(`http://localhost:3000/recipes/?name=${name}`)
            return dispatch({
...
export function getRecipeDetails(payload) {
    return async function(dispatch) {
        try {
            const response = await axios.get(`http://localhost:3000/recipes/${payload}`);
            return dispatch({
...
export function createRecipe(payload) {
    return async function(dispatch) {
        try {
            var response = await axios.post(`http://localhost:3000/recipe`, payload);
            return dispatch({
                type: "CREATE_RECIPE",
                payload: response,
            })
